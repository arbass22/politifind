...
from politifind.models import User, UserVote, Bill, UserPoliticianSubscription, Politician, UserBillSubscription, Committee, UserCommitteeSubscription
...
def run():
    ...
    for u in users:
        ...
        while(i < z):
            ...
            uv = UserVote(uid=User.objects.get(uid=u.uid), bid=Bill.objects.get(bid=bills[r_index1].bid), vote=v, date_voted=getADate())
            uv.save()
        y = randrange(1,5)
        i = 0
        while(i < y):
            ...
            ups = UserPoliticianSubscription(uid=User.objects.get(uid=u.uid), pid=Politician.objects.get(pid=ps[r_index2].pid), date_subscribed=getADate())
            ups.save()
        i = 0
        w = randrange(1,5)
        while(i < w):
            ...
            ubs = UserBillSubscription(uid=User.objects.get(uid=u.uid), bid=Bill.objects.get(bid=bills[r_index3].bid), date_subscribed=getADate())
            ubs.save()
        i = 0
        k = randrange(1, 5)
        while(i < k):
            ...
            ucs = UserCommitteeSubscription(uid=User.objects.get(uid=u.uid), cid=Committee.objects.get(cid=cs[r_index4].cid), date_subscribed=getADate())
            ucs.save()
